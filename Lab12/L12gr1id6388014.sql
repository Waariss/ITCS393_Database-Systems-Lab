...
-- Q2 Create a stored procedure named sp _Orders_By_Customer 
-- “ that takes a customer number (@customerNum) as input and returns a list of orders for that customer.
DELIMITER //
...
-- old_rep_num and new_rep_num. The function should update the Rep table to change 
-- the RepNum value from old_rep_num to new_rep_num. It should also update the 
-- Customer and CurrentOrders tables to reflect the change. 
-- If old_rep_num does not exist in the Rep table, the function should return false (0). 
-- Otherwise, it should return true (1)
DELIMITER //
